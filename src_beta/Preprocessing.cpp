...
void PreprocessMatrices(RawMatrix** matrices, Trial* buf_trials, Trial* trials, int nSubs, int nTrials)
{
  ...
  {
    int sid=buf_trials[i].sid;
    new_cols[sid]+=buf_trials[i].ec-buf_trials[i].sc+1;
  }
  ...
  for (int i=0; i<nSubs; i++)
  {
    ...
    {
      if (buf_trials[j].sid==i)
      {
        int sc= buf_trials[j].sc;
        int ec= buf_trials[j].ec;
        ...
        for (int r=0; r<row; r++)
        {
          ...
          for (int c=sc; c<=ec; c++)
          {
            buf[cur_col*row+r*delta_col+c-sc] = (matrix[r*col+c] - mean_f) * inv_sd_f; // if sd is zero, a "nan" appears
          }
        }
        trials[j].sc = cur_col;
        trials[j].ec = delta_col;
        cur_col = cur_col+delta_col;
      }
    }
    delete matrix;
