...
// don't forget shift 2!! we can speficy the shifting in the block file, but we
// don't use shifting for face/scene dataset

/***********************************
main function of this file, do the traditional feature selection and
classification
input: the averaged raw matrix array, the number of subjects, the trials, the
number of trials, the number of test samples, the number of folds in the svm-cv,
the voxel location information, the top voxel file name, the mask file name
output: write the voxels in decreasing order of classification accuracy to the
top voxel file\
************************************/
void Searchlight(RawMatrix** avg_matrices, int nSubs, Trial* trials,
  ...
  int i;
  int row = masked_matrices[0]->row;  // assume all elements in r_matrices array
                                      // have the same row
  std::cout << "#voxels for mask: " << row << std::endl;
  VoxelScore* scores = GetSearchlightSVMPerformance(
      masked_matrices, trials, nTrials, nTests, nFolds, masked_pts);
  ...
  ofile.close();
  int* data_ids =
      (int*)GenerateNiiDataFromMask(maskFile, scores, row, DT_SIGNED_INT);
  sprintf(fullfilename, "%s", topVoxelFile);
  strcat(fullfilename, "_seq.nii.gz");
  WriteNiiGzData(fullfilename, maskFile, (void*)data_ids, DT_SIGNED_INT);
  float* data_scores =
      (float*)GenerateNiiDataFromMask(maskFile, scores, row, DT_FLOAT32);
  sprintf(fullfilename, "%s", topVoxelFile);
  strcat(fullfilename, "_score.nii.gz");
  WriteNiiGzData(fullfilename, maskFile, (void*)data_scores, DT_FLOAT32);
  delete[] scores;
  if (maskFile != NULL) delete masked_pts;
#endif
}

/****************************************
get the SVM results of classifying activation vectors of two categories for
every voxel
the linear kernel of libSVM is applied here
input: the average activation matrix array, the blocks(trials), the number of
blocks, the number of test samples, the number of folds in the cross validation,
the location info
output: a list of voxels' scores in terms of SVM accuracies
*****************************************/
VoxelScore* GetSearchlightSVMPerformance(
    RawMatrix** avg_matrices, Trial* trials, int nTrials, int nTests,
    int nFolds, VoxelXYZ* pts)  // classifiers for a r_matrix array
{
  svm_set_print_string_function(&print_null);
  int row = avg_matrices[0]->row;  // assume all elements in r_matrices array
                                   // have the same row
  VoxelScore* score =
      new VoxelScore[row];  // get step voxels classification accuracy here
  int i;
#pragma omp parallel for private(i) schedule(dynamic)
  for (i = 0; i < row; i++) {
    SVMProblem* prob = GetSearchlightSVMProblem(avg_matrices, trials, i,
                                                nTrials - nTests, pts);
...
input: the average activation matrix array, the number of blocks, the blocks,
the current voxel id, the number of training samples, the location info
...
SVMProblem* GetSearchlightSVMProblem(RawMatrix** avg_matrices, Trial* trials,
  ...
  for (int i = 0; i < nTrainings; i++) {
    // cout<<trials[i].tid<<" "<<trials[i].sid<<" "<<trials[i].label<<"
    // "<<trials[i].sc<<" "<<trials[i].ec<<endl;
    ...
    for (j = 0; j < nSphereVoxels; j++) {
      ...
      if (voxels[j] != -1) {
        prob->x[i][j].value =
            avg_matrices[sid]->matrix[voxels[j] * col + offset];
      } else {
        prob->x[i][j].value = 0;
      }
