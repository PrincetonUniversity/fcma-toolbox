...
input: starting time point, end time point, number of voxels, number of total
time points, the raw matrix, normalized matrix
output: write results to the normalized matrix, return the length of time points
to be computed
******************************/
int getBuf(int start_col, int end_col, int row, int col, float* mat,
  ...
  for (i = 0; i < row; i++) {
    ...
    for (int j = start_col; j <= end_col; j++) {
      ...
      sd += (double)mat[i * col + j] *
            mat[i * col + j];  // convert to double to avoid overflow
    }
    mean /= delta_col;
...
#pragma simd
    for (int j = start_col; j <= end_col; j++) {
      buf[i * delta_col + j - start_col] =
          (mat[i * col + j] - mean_f) *
          inv_sd_f;  // bds 1/sd; // if sd is zero, a "nan" appears
    }
  }
  return delta_col;
}
...
input: a trial struct, starting row id, step (the row of the correlation matrix,
whose column is row), the raw matrix struct array
...
CorrMatrix* CorrMatrixComputation(Trial trial, int sr, int step,
  ...
  int col = matrices1[sid]->col;  // the column of 1 and 2 should be the same,
                                  // i.e. the number of TRs of a block
  ...
  float* buf2 =
      new float[row2 * col];  // col is more than what really need, just in case
  int ml1 = getBuf(sc, ec, row1, col, mat1, buf1);  // get the normalized
                                                    // matrix, return the length
                                                    // of time points to be
                                                    // computed
  /* int ml2 = */ getBuf(sc, ec, row2, col, mat2,
                         buf2);  // get the normalized matrix, return the length
                                 // of time points to be computed, m1==m2
  ...
  // cblas_sgemm(CblasRowMajor, CblasNoTrans, CblasTrans, step, row2, ml1, 1.0,
  // buf1+sr*ml1, ml1, buf2, ml2, 0.0, corrs, row2);
  matmul(buf1 + sr * ml1, buf2, corrs, step, ml1, row2);
  // if (sr==0 && sid==2 && sc==4) {cout<<corrs[99]<<" "<<corrs[100]<<endl;
  // exit(1);}
...
input: the trial struct array, number of trials, the starting row (since
distributing to multi-nodes, step the raw matrix struct array
...
#pragma omp parallel for private(i)
  for (i = 0; i < nTrials; i++) {
    c_matrices[i] =
        CorrMatrixComputation(trials[i], sr, step, matrices1, matrices2);
  }
  return c_matrices;
}
