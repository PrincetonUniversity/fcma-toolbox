...
inline void z_score(float* v, int n) {
  int i;
  ALIGNED(64) float mean = 0,
                    sd = 0;  // float here is not precise enough to handle
#pragma loop count(12)
  ...
  mean /= n;
  sd = sd / n - mean * mean;
  ALIGNED(64) float inv_sd =
      sd <= 0 ? 0.0 : 1 / sqrt(sd);  // do time-comsuming division once, sd==0
                                     // means all values are the same, sd<0
                                     // happens due to floating point number
                                     // rounding error
#pragma loop count(12)
#pragma simd
  for (i = 0; i < n; i++) {
...
RawMatrix** rawMatPreprocessing(RawMatrix** r_matrices, int n, int nTrials,
                                Trial* trials);
