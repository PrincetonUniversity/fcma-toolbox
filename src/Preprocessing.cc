...
/**************************
align multi-subjects, remove all-zero voxels, if all-zero in one subject, remove
the corresponding voxel among all subjects
input: the raw matrix data structure array, the number of matrices (subjects)
output: the remaining number of voxels, remove voxels from raw matrix and
location
***************************/
int AlignMatrices(RawMatrix** r_matrices, int nSubs, VoxelXYZ* pts) {
  ...
  for (i = 0; i < nSubs; i++) {
    ...
    for (j = 0; j < row; j++) {
      ...
      for (k = 0; k < col; k++) {
        flag &= (mat[j * col + k] <=
                 10.0);  // 10 is a threshold for "almost" all-zero
      }
...
/**************************
align multi-subjects, using a file containing a binary vector that indicates
whether or not to use a voxel
input: the raw matrix data structure array, the number of matrices (subjects),
the binary vector file
output: the remaining number of voxels, remove voxels from raw matrix and
location
***************************/
int AlignMatricesByFile(RawMatrix** r_matrices, int nSubs, const char* file,
                        VoxelXYZ* pts) {
#ifndef __MIC__
  // align multi-subjects, remove all-zero voxels, assume that all subjects have
  // the same number of voxels
  int row =
      r_matrices[0]->row;  // originally is the total number of voxels x*y*z
  int i, j;
  int count = 0;
  nifti_image* nim;
...
input: the trial array, the number of trials, the starting leave-out trial id,
the number of trials that left out
...
/****************************************
Fisher transform the correlation values (coefficients) then z-scored them across
blocks
input: the correlation matrix array, the length of this array (the number of
blocks), the number of subjects
...
void corrMatPreprocessing(CorrMatrix** c_matrices, int n, int nSubs) {
  ...
  int col = c_matrices[0]->nVoxels;  // assume that all correlation matrices
                                     // have the same size
  int i;
  if (n % nSubs != 0) {
  ...
  // doing this can make better cache usage than going through voxel pairs in
  // the outtest loop
...
/****************************************
average the activation values by blocks then z-scored them across blocks within
subjects
input: the raw matrix array, the length of this array (the number of subjects),
the number of blocks per subject, the trials
output: the average values of all subjects after z-scoring
*****************************************/
RawMatrix** rawMatPreprocessing(RawMatrix** r_matrices, int n,
                                int nTrialsPerSub, Trial* trials) {
  int row = r_matrices[0]->row;  // assume that all correlation matrices have
                                 // the same size
  ...
  for (i = 0; i < n; i++) {
    ...
    for (j = 0; j < row; j++) {
      for (k = 0; k < nTrialsPerSub; k++) {
        avg_matrices[i]->matrix[j * nTrialsPerSub + k] =
            getAverage(r_matrices[i], trials[k], j);
      }
...
/****************************************
permute voxels within subject based on an input file or a given seed
randomization
input: the raw matrix array, the length of this array (the number of subjects),
the randomization seed, the permuting file
output: voxels have been randomly permuted
*****************************************/
void MatrixPermutation(RawMatrix** r_matrices, int nSubs, unsigned int seed,
  ...
  for (i = 0; i < nSubs; i++) {
    ...
    for (j = 0; j < row; j++) {
      k = index[j];
      memcpy((void*)buf, (const void*)&(r_matrices[i]->matrix[j * col]),
             col * sizeof(float));
      memcpy((void*)&(r_matrices[i]->matrix[j * col]),
             (const void*)&(r_matrices[i]->matrix[k * col]),
             col * sizeof(float));
      memcpy((void*)&(r_matrices[i]->matrix[k * col]), (const void*)buf,
             col * sizeof(float));
    }
  }
  if (permute_book_file) {
...
/****************************************
use the trials information to filter the given raw matrices and normalize it for
correlation computation
input: the raw matrix array, the buffer trial array storing old trial info, the
trial array, the length of raw matrix array (the number of subjects), the length
of trials array (the number of trials)
output: the matrix array is rewritten to be one by one submatrix instead of one
big matrix and the trial array is modified accordingly
*****************************************/
TrialData* PreprocessMatrices(RawMatrix** matrices, Trial* trials, int nSubs,
                              int nTrials) {
  assert(nTrials > 1);
  assert(matrices);
  assert(matrices[0]);
...
#pragma omp parallel for
  for (int i = 0; i < nTrials; i++) {
    ...
    for (int r = 0; r < row; r++) {
      double mean = 0.0f,
             sd = 0.0f;  // float here is not precise enough to handle
