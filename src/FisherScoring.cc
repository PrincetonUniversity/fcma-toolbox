...
void GetSAndH(float* data, float* data2, float* beta, int length, int* labels,
...
#pragma simd
    for (i = 0; i < length; i++) {
      ...
      // if (isinf(e)) {coef=1.0; coef2=0.0;}  // this branch harms the
      // performance of MIC more than host
      s0[i] = y - coef;
...
// in the vectorization+reduction case, if the compiler is not sure about the
// dependency (do S and H overlap?),
...
float* GetInverseMat(float* mat, int rank) {
  ...
  if (rank == 2) {
    float det = mat[0 * rank + 0] * mat[1 * rank + 1] -
                mat[0 * rank + 1] * mat[1 * rank + 0];
    if (det == 0) {
    ...
    float f = mat[8];
    float det =
        -1 / (a * d * f - a * e * e - b * b * f + 2 * b * c * e - c * c * d);
    if (det == 0) {
      cerr << a << " " << b << " " << c << " " << d << endl;
      cerr << mat[0] << " " << mat[1] << " " << mat[2] << " " << mat[4] << " "
           << mat[5] << " " << mat[8] << endl;
      cerr << "matrix cannot be inversed!" << endl;
      det = 1;
    }
...
float DoIteration(float* data, float* data2, int length, int* labels,
  ...
  if (rank == 2) {
    ...
    while (lambda > epsilon) {
      ...
      lambda = -((S[0] * iH[0] + S[1] * iH[2]) * S[0] +
                 (S[0] * iH[1] + S[1] * iH[3]) * S[1]);
...
float DoIteration2(float* dataHead, int offset1, float* dataHead2, int offset2,
  ...
  if (rank == 2) {
    ...
    while (lambda > epsilon) {
      ...
      if (H[2] == 0 && H[3] == 0) {
        for (int ii = 0; ii < length; ii++) std::cerr << data[ii] << " ";
        std::cerr << std::endl;
