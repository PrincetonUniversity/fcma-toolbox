...
/****************************************
get the distance ratios between correlation vectors within the same category and
across categories for every voxel
the distance ratio to be computed here is not based on the Euclidean distance
but something that is similar to the square of Euclidean distance
...
*****************************************/
VoxelScore* GetDistanceRatio(int me, CorrMatrix** c_matrices,
                             int nTrials)  // get scores for a c_matrix array
{
...
#pragma omp parallel for
  for (int i = 0; i < length; i += row) {
    ...
    (scores + count)->score =
        1 - DoDistanceRatioSmarter(nTrials, i, c_matrices, row);
  }
  return scores;
}

/*****************************************
get the distance ratio between correlation vectors within the same category and
across categories for one voxel
...
float DoDistanceRatioSmarter(int nTrainings, int startIndex,
  ...
  for (i = 0; i < nTrainings; i++) {
    if (c_matrices[i]->tlabel == 1) {
      for (j = 0; j < length; j++) {
        ...
        c0_2[j] += c_matrices[i]->matrix[startIndex + j] *
                   c_matrices[i]->matrix[startIndex + j];
      }
      nIn0++;
    } else {
      for (j = 0; j < length; j++) {
        c1[j] += c_matrices[i]->matrix[startIndex + j];
        c1_2[j] += c_matrices[i]->matrix[startIndex + j] *
                   c_matrices[i]->matrix[startIndex + j];
      }
      nIn1++;
    }
  ...
  float ratio =
      ((dIn0 + dIn1) / ((nIn0 - 1) * nIn0 / 2 + (nIn1 - 1) * nIn1 / 2)) /
      (dOut / (nIn0 * nIn1));
